...
`index.html#highscores` shows high scores, top earners from runners, biggest contributors, who pays whom and the most recent transactions. It reads independently on entry and also receives live game-sync updates.
...
Portfolio payment traces use new confirmed ledger rows: one red outgoing or green incoming pulse per player connection per sync, lasting 950ms. Entry, reconnection, hidden views and off-screen cards never replay payments; reduced motion skips traces. The overlays do not affect layout, balances or Sector 7. `test/portfolio-payments.js` covers these interactions against the local shared-sheet mock.
